...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/">
        ...
        <xsl:for-each select="$unitCoverageMisses/*[not(@method=$excluded/excludedMethod/@name or @class=$excluded/excludedClass/@name or ends-with(@class,'Enum'))]">
ERROR: unit coverage miss at <xsl:value-of select="@method"/>
        </xsl:for-each>
        <xsl:for-each select="$mutationCoverageMisses/*[not(@method=$excluded/excludedMethod/@name or @class=$excluded/excludedClass/@name or ends-with(@class,'Enum'))]">
