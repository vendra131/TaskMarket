...
<xsl:stylesheet version="2.0"
	...
	<xsl:function name="zenta:fullpackageP">
		...
		<xsl:if test="$package">
			<xsl:variable name="parent"
				select="zenta:fullpackageP(zenta:neighbours($rich,$package,'contains,2')[@xsi:type='Package'])" />
			<xsl:choose>
				<xsl:when test="$parent">
					<xsl:value-of
						select="concat($parent, '.', $package/@name)" />
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:function name="zenta:fullpackage">
		...
		<xsl:variable name="parents"
			select="zenta:neighbours($rich,$service,'contains,2')" />
		<xsl:variable name="parent"
			select="if($parents[@xsi:type='Package'])
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="implemented"
					select="zenta:neighbours($rich,$service,'is implemented by/implements,2')" />
