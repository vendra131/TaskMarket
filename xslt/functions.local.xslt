...
<xsl:stylesheet
    ...
    <xsl:function name="zenta:taskmarketErrorDescription">
...
Behaviour: <xsl:value-of select="$testcase/@feature"/>/<xsl:value-of select="$testcase/@operation"/>;<xsl:value-of select="$testcase/@testcase"/>
 ...
 @TestedFeature("<xsl:value-of select="$testcase/@feature"/>")
</para><para>
 @TestedOperation("<xsl:value-of select="$testcase/@operation"/>")
</para><para>
 @TestedBehaviour("<xsl:value-of select="$testcase/@testcase"/>")
</para><para>

<xsl:value-of select="$testcase"/>
</para><para>
    	[Deviation in model](http://repo.kodekonveyor.com/TaskMarket/KodeKonveyor/master/index.html#<xsl:value-of select="zenta:taskmarketErrorId($entry)"/>)
</para>
    		</xsl:when>
    		<xsl:otherwise>
    	issue markup:
    	[Deviation in model](http://repo.kodekonveyor.com/TaskMarket/KodeKonveyor/master/index.html#<xsl:value-of select="$entry//object/testcase/@name"/>)
    		</xsl:otherwise>
    	</xsl:choose>
    </xsl:function>
    <xsl:function name="zenta:taskmarketErrorId">
    	<xsl:param name="entry"/>
    	<xsl:variable name="testcase" select="$entry/object/testcase"/>
    	<xsl:choose>
    		<xsl:when test="$testcase/@featureid">
    ...
    <xsl:function name="zenta:taskmarketErrorTitle">
    	<xsl:param name="entry"/>
    	<xsl:variable name="testcase" select="$entry/object/testcase"/>
