...
<xsl:copy-of select="@name"/>,
            ...
            <xsl:for-each select="saxon:evaluate(@xpath,$doc,$element)">
                <value derived="true">
				    ...
				    <xsl:variable name="targetType" select="$doc//element[@id = $templateRelation/connection/@target and @name=$intersection]"/>
				    <xsl:variable name="ancestorId" select="$templateRelation/connection[@target=$targetType/@id]/@id"/>
                    ...
                    <xsl:attribute name="ancestor" select="$ancestorId"/>
		    <xsl:attribute name="id" select="concat($element/@id,'-',$ancestorId,'-',@id)"/>
                </value>
            </xsl:for-each>
        </xsl:for-each>
    </xsl:function>
